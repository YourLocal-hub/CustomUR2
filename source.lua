...
local requestsDisabled = true --getgenv and getgenv().DISABLE_CUSTOMUR2_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.0.0"
local RayfieldFolder = "CUSTOMUR2"
local ConfigurationFolder = CustomUR2Folder.."/Configurations"
local ConfigurationExtension = ".curld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'CustomUR2 Keybind'},
		-- buildwarnings
		-- customur2prompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.CustomUR2Open"] = "J"
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(CustomUR2Folder) then
				if isfile and isfile(CUSTOMUR2Folder..'/settings'..ConfigurationExtension) then
					file = readfile(CUSTOMUR2Folder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"CustomUR2Open":{"Value":"K","Type":"bind","Name":"CustomUR2 Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"CustomUR2 Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('CustomUR2 had an issue accessing configuration saving capability.')
		end
	end
end
...
--		warn('CustomUR2 Interface | CustomUR2 Analytics:\n\n\nAs you don\'t have file functionality with your executor, we are unable to save whether you want to opt in or out to analytics.\nIf you do not want to take part in anonymised usage statistics.')
--		analytics = true	
--	else
--		prompt.create(
--			'Help us improve',
--	            [[Would you like to allow CustomUR2 to collect usage statistics?
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("CustomUR2", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify CustomUR2 developers.")
				end
			end)
...
local Rayfield = useStudio and script.Parent:FindFirstChild('CustomUR2') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local rayfieldDestroyed = false -- True when CustomUR2Library:Destroy() is called

repeat
	if Rayfield:FindFirstChild('Build') and CustomUR3.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('CustomUR2 | Build Mismatch')
		print('CustomUR2 may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of Rayfield is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, CustomUR2 = CustomUR2, useStudio and script.Parent:FindFirstChild('CustomUR2') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(CustomUR2)
	Rayfield.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	CustomUR2.Parent = CoreGui:FindFirstChild("RobloxGui")
elseif not useStudio then
	CustomUR2.Parent = CoreGui
end

if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Rayfield.Name and Interface ~= CustomUR2 then
			Interface.Enabled = false
			Interface.Name = "CustomUR2-Old"
		end
	end
elseif not useStudio then
	for _, Interface in ipairs(CoreGui:GetChildren()) do
		if Interface.Name == CustomUR2.Name and Interface ~= CustomUR2 then
			Interface.Enabled = false
			Interface.Name = "CustomUR2-Old"
		end
	end
...
if Rayfield.AbsoluteSize.X < minSize.X and CustomUR2.AbsoluteSize.Y < minSize.Y then
...
local Main = CustomUR2.Main
local MPrompt = CustomUR2:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = CustomUR2:FindFirstChild('Drag')
...
local Notifications = CustomUR2.Notifications
